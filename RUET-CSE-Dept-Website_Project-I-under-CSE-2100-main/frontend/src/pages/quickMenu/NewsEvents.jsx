...
const NewsEvents = () => {
  ...
  return (
    <>
      <Breadcrumb current="News & Events" />

      <PageHeroBanner
        title="News & Events"
        navLinks={SUB_NAV}
        activeTo="/news-events"
      />
        ...
        <div className="flex flex-col">
          ...
          <div className="flex gap-0">
            ...
            <div className="flex-1 min-w-0 border border-light-blue border-t-0"
              style={{
                padding: currentItems.length === 0
                  ? undefined
                  : 'clamp(1.25rem, 3vw, 3rem) clamp(1rem, 3vw, 3rem)',
              }}
            >
              {currentItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center text-gray-blue py-20 gap-3 font-poppins">
                  ...
                  <button
                    onClick={handleReset}
                    className="mt-1 text-blue border border-blue px-4 py-1.5 text-sm hover:bg-blue/10 transition-colors"
                  >
                    Clear Filters
                  </button>
                </div>
...
//   const totalPages = Math.max(1, Math.ceil(newsEvents.length / ITEMS_PER_PAGE))
//   const safePage   = Math.min(currentPage, totalPages)

//   const currentItems = useMemo(() => {
//     const start = (safePage - 1) * ITEMS_PER_PAGE
//     return newsEvents.slice(start, start + ITEMS_PER_PAGE)
//   }, [safePage])
...
//         style={{
//           paddingTop:   'clamp(2.5rem, 5vw, 6rem)',
//           paddingLeft:  'clamp(1rem, 9vw, 10rem)',
//           paddingRight: 'clamp(1rem, 9vw, 10rem)',
//         }}
//       >
//         {/* Card Grid */}
//         <div
